...
def get_number_class_failures(student_list):
    ...
    for student in student_list:
        for course, failed_number in student_list.classes_failed:
            class_number_failed.setdefault(course,0)
            class_number_failed[course]+=1
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <3 or len(sys.argv) > 4:
        ...
        for i in range(num_iterations):
            ...
            dropout_ids.sort()
            minor_ids.sort()
            average_time=get_average_time_in_system(graduates)
            average_fail = get_average_classes_failed(graduates)
            #classes_failed=class_number_failed(students)
